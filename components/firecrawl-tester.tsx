...
export default function FirecrawlTester() {
  ...
  const onRun = async () => {
    try {
      ...
      const url = "https://www.olx.pl/zwierzeta/konie/?page=2&search%5Border%5D=created_at%3Adesc";
      ...
      if (data?.offerUrls && data.offerUrls.length > 0) {
        ...
        const offerDetails = await scrapeOfferDetails({ offerUrl: firstOfferUrl });

...
const horseOfferSchema = z.object({
  ...
  gender: z.string().optional().describe("Płeć konia (Ogier/Klacz/Wałach/Źrebak/Źrebica)"),
  ...
  coordinates: z.object({
    lat: z.number(),
    lng: z.number(),
  }).optional().describe("Współrzędne geograficzne"),
  
  // Horse details
  purpose: z.string().optional().describe("Przeznaczenie konia (sportowy/rekreacyjny/roboczy)"),
  ...
  healthStatus: z.enum(["healthy", "injured", "unrideable", "unknown"]).optional().describe("Stan zdrowia"),
  ...
  }).optional().describe("Informacje kontaktowe"),
...
export const fetchTestData = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Firecrawl request failed: Firecrawl error ${response.status}: ${text}`);
    }
...
export const scrapeOfferDetails = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Firecrawl offer request failed: Firecrawl error ${response.status}: ${text}`);
    }
